...
- **재구축 F1–F9·F11·F12·F13·F14 구현 완료.** `Source/` 5개 패키지(Attribute·Shared·CodeGenerator·Client·Server), `Sandbox/` 3개, `Test/` 3계층(132개 통과) + 벤치마크 1(DRPC.Benchmarks, [[../03-Reference/Performance|Performance]] 기준선).
...
- 미구현: F10 TemplateSource. (릴리스: … → `v2.9.2`(MessageProtocol 2.3.7 채택, patch) → `v2.10.0`(호출별 타임아웃 정책, minor) → `v2.11.0`(패킷 암호화 F13 + Comm 2.5.0·MP 2.3.9 채택, minor) → `v2.12.0`(구현 전 검증 게이트 F14, minor) → `v2.13.0`(생성 허브 `RpcEndpointOptions` 오버로드 — 옵션 사용 시에도 간단 경로, minor) — 5개 패키지 NuGet 게시 확인)
