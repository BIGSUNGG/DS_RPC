...
- 미구현: F10 TemplateSource. (릴리스: … → `v2.11.0`(패킷 암호화 F13 + Comm 2.5.0·MP 2.3.9 채택, minor) → `v2.12.0`(구현 전 검증 게이트 F14, minor) → `v2.13.0`(생성 허브 `RpcEndpointOptions` 오버로드 — 옵션 사용 시에도 간단 경로, minor) — 5개 패키지 NuGet 게시 확인. 이후 2026-09-11 **MP 3.0.0 마이그레이션(코드 전용 — DRPC 패키지 버전 무변동)** + 같은 날 **Comm 2.5.1 채택(패치, API 무변화)**)
