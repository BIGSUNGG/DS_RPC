...
- **재구축 F1–F7·F9·F11·F12 구현 완료.** `Source/` 5개 패키지(Attribute·Shared·CodeGenerator·Client·Server), `Sandbox/` 3개, `Test/` 3계층(99개 통과).
- 형제 스택은 **NuGet 안정판만** 참조한다(`MessageProtocol` **2.3.1**, `Communication.Network.RUDP.*`·`Communication.Shared` **2.2.1** — CRC32c 무결성·흐름제어·프레임 상한·ConnectTimeout 포함) — 형제 저장소 프로젝트 참조·하드 경로 없음.
...
- 미구현: F10 TemplateSource. (릴리스: `v2.0.0` → `v2.1.0`(F12) → `v2.2.0`(형제 채택+타임아웃) → `v2.3.0`(고갈 방어 3종) → `v2.4.0`(호출 취소) → `v2.5.0`(형제 2차 채택+CRC32c·RpcEndpointOptions) — 5개 패키지 NuGet 게시 확인)
