...
- **재구축 F1–F7·F9·F11·F12 구현 완료.** `Source/` 5개 패키지(Attribute·Shared·CodeGenerator·Client·Server), `Sandbox/` 3개, `Test/` 3계층(111개 통과) + 벤치마크 1(DRPC.Benchmarks, [[../03-Reference/Performance|Performance]] 기준선).
...
- 미구현: F10 TemplateSource. (릴리스: … → `v2.9.0`(P4 운영 신호) → `v2.9.1`(핫패스 할당 제거·Comm 2.4.0, patch) → `v2.9.2`(MessageProtocol 2.3.7 채택, patch) — 5개 패키지 NuGet 게시 확인)
