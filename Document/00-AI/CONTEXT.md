...
- **재구축 F1–F7·F9·F11·F12 구현 완료.** `Source/` 5개 패키지(Attribute·Shared·CodeGenerator·Client·Server), `Sandbox/` 3개, `Test/` 3계층(91개 통과).
- 형제 스택은 **NuGet 안정판만** 참조한다(`MessageProtocol` **2.3.0** — GenericMessage 포함, `Communication.Network.RUDP.*`·`Communication.Shared` **2.0.1** — RUDP 흐름제어·프레임 상한·ConnectTimeout 포함) — 형제 저장소 프로젝트 참조·하드 경로 없음.
...
- 미구현: F10 TemplateSource. (릴리스: `v2.0.0` → `v2.1.0`(F12 제네릭 프로시저) → `v2.2.0`(형제 채택 + 연결 타임아웃) — 5개 패키지 NuGet 게시 확인)
